    ...
    try:
        ...
        try:
            with open(filename, 'r') as f:
                ...
                print(f"Successfully loaded {len(saved_devices)} saved device names from {filename}")
                return saved_devices
        except (FileNotFoundError, json.JSONDecodeError) as e:
            print(f"No saved device names found or invalid JSON in {filename}: {e}")
            return {}
    except Exception as e:
...
def scan_network():
    ...
    print(f"Loaded {len(saved_device_names)} saved device names")
    ...
    try:
        ...
        for host in nm.all_hosts():
            ...
            # Check if we have a saved hostname for this IP
            if host in saved_device_names:
                hostname = saved_device_names[host]['hostname']
                print(f"Using saved hostname for {host}: {hostname}")
            ...
            # Use saved MAC address if available
            if host in saved_device_names and device_info['mac'] == 'Unknown':
                ...
                if saved_mac and saved_mac != 'Unknown':
                    ...
                    print(f"Using saved MAC for {host}: {saved_mac}")
...
# Function to save device changes to a JSON file
def save_device_changes(device):
    """Save device hostname and MAC changes to a JSON file based on the network"""
    try:
        network_info = get_local_network_info()
        gateway_ip = network_info['gateway_ip'].replace('.', '_')
        local_ip = network_info['local_ip'].replace('.', '_')
        ...
        print(f"Saved device changes for {device['ip']} to {filename}")
